    ...
    <div class="row">
        <div class="col-xs-12">
           ...
           <div class="box-header">
                     <h3 class="box-title">Denomination Setup Detail</h3>
                </div>
                    ...
                    <div class="container-fluid">
                        <div class="row">
                            <div class="form-group">
                                <label class="col-md-2 control-label"><strong>Package Code&ast;</strong></label>
                                <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="form-group">
                                <label class="col-md-2 control-label"><strong>Denomination Name&ast;</strong></label>
                                <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="form-group">
                                <label class="col-md-2 control-label"><strong>Denomination&ast;</strong></label>
                                <div class="col-md-6">
                                    <input type="text" id="denomination" name="denomination" class="form-control state_edit   " autocomplete="off" value="" maxlength="100" data-error="This field is required." required>
                                    <div class="help-block with-errors"></div>
                                    <label id="denomination_view" class="state_view"></label>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group">
                                <label class="col-md-2 control-label"><strong>Denomination Type&ast;</strong></label>
                                <div class="col-md-4">
                                    <div class="denomType_setup state_edit"><select class="form-control"></select></div>
                                    <label id="denomType_setup_view" class="state_view"></label>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        function submitData(){
            var value = {
                ...
                "denomTypeCode": $('#denomType_list').val(),
                "denomTypeName": $("#denomType_list option:selected").text(),
            };
    ...
    function getdenomTypeDropList() {

        var url_action = 'searchDenomTypeForDroplist';
        var action = 'SEARCH';
        var menu = 'MNU_GPCASH_MT_PREPAID_DENOM';
        $.ajax({
            url: 'getAPIData',
            method: 'post',
            type: 'json',
            ...
            success: function (data) {
                var result = JSON.parse(data);
                if (result.status=="200") {
                    taxOption = '<select id="denomType_list" class="form-control state_edit" required>';
                    taxOption += '<option value="" selected="selected">--Select Denomination Type--</option>';
                    $.each(result.result, function (idx, obj) {
                        taxOption += '<option value="' + obj.code + '">'+ obj.code +' - '+ obj.name + '</option>';

                    });
                    taxOption += '</select>';
                    taxOption += '<div class="help-block with-errors"></div>';
                } else {
                    flash('warning', result.message);
                }
                ...
                $('.denomType_setup').html(taxOption);
            }
        });
    }

    function getDenomEdit(code_id){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.status=="200") {
                    ...
                    $('#denomination').val(detail.denomination);
                    $('#denomType_list').val(detail.denomTypeCode).trigger('change');
                } else {
                    flash('warning', result.message);
                }
        ...
        function stateView() {
            ...
            var denomination = ($('#denomination').val() == '' ? '-' : $('#denomination').val());
            var denomType = ($('#denomType_list option:selected').text() == '' ? '-' : $('#denomType_list option:selected').text());
            ...
            $('#name_view').text(name);
            $('#dscp_view').text(dscp);
            $('#denomination_view').text(denomination);
            $('#denomType_setup_view').text(denomType);
            $('#save_screen').hide();
            $('.help-block').hide();
            $('#done').hide();
            $('#next_user').hide();
