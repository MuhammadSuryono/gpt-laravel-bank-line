           ...
           <div class="box-header">
                ...
                <div class="box-body">
                    <div class="container-fluid">
                        ...
                        <div class="row">
                            <div class="form-group">
                                <label class="col-md-4 control-label lblcustID"><strong>Customer ID&ast;</strong></label>
                                <div class="col-md-5">
									<select id="customer_list" class="form-control state_edit" data-error="Customer ID is mandatory" required>
										<option></option>
									</select>
									<label id="customer_setup_view" class="state_view"></label>
                                </div>
                            </div>
                        </div>
                        <div class="row local">
...
<script>
    ...
    function getCustomer() {
        ...
        var action = 'SEARCH';
        //var menu = service;
        var menu = 'MNU_R_GPCASH_BO_RPT_TRX_STS'; 
        ...
        $.ajax({
            ...
            success: function (data) {
                var result = JSON.parse(data);
                if (result.status=="200") {
                    //unitOption = '<select id="customer_list" class="form-control state_edit" data-error="Customer ID is mandatory" required>';
                    unitOption = '<option value=""></option>';
                    $.each(result.result, function (idx, obj) {
                        unitOption += '<option value="' + obj.customerId + '">'+ obj.customerId + ' - ' + obj.customerName + '</option>';
                    });
                    //unitOption += '</select>';
                    //unitOption += '<div class="help-block with-errors"></div>';

                    $('#customer_list').html(unitOption);
					//$('#customer_list').select2();	
					//$('#customer_list').className("state_edit");
    ...
    function getRateEdit(code_id){
        ...
        function stateView() {
            ...
            var customerId = ($('#customer_list option:selected').text() == '' ? '-' : $('#customer_list option:selected').text());
            ...
            $('#customer_setup_view').text(customerId);
        ...
        function changeLabel(isBack){
            ...
            if (isBack) {
                ...
                $('.lblcustID').html('<strong>Customer ID*</strong>');
