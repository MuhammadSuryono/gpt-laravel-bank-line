    ...
    <div class="row">
        <div class="col-xs-12">
           ...
           <div class="box-header">
                     <h3 class="box-title">Deposit Setup Detail</h3>
                </div>
                    ...
                    <div class="container-fluid">
                        <div class="row">
                            <div class="form-group">
                                <label class="col-md-2 control-label"><strong>Deposit Code&ast;</strong></label>
                                <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="form-group">
                                <label class="col-md-2 control-label"><strong>Deposit Name&ast;</strong></label>
                                <div class="col-md-6">
                        ...
                        <div class="row">
                            <div class="form-group">
                                <label class="col-md-2 control-label"><strong>Tax Deposit Type Code&ast;</strong></label>
                                <div class="col-md-6">
                                    <input type="text" id="taxDepositType" name="taxDepositType" class="form-control state_edit   " autocomplete="off" value="" maxlength="100" data-error="This field is required." required>
                                    <div class="help-block with-errors"></div>
                                    <label id="taxDepositType_view" class="state_view"></label>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group">
                                <label class="col-md-2 control-label"><strong>Tax Type&ast;</strong></label>
                                <div class="col-md-4">
                                    <div class="taxType_setup state_edit"><select class="form-control"></select></div>
                                    <label id="taxType_setup_view" class="state_view"></label>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        function submitData(){
            var value = {
                ...
                "name": $('#name').val(),
                "dscp": $('#dscp').val(),
                "taxDepositCode": $('#taxDepositType').val(),
                "taxTypeCode": $('#taxType_list').val(),
                "taxTypeName": $("#taxType_list option:selected").text(),
            };
    ...
    function getTaxTypeDropList() {

        var url_action = 'searchTaxTypeForDroplist';
        var action = 'SEARCH';
        var menu = 'MNU_GPCASH_TAX_DEPOSIT_TYP';
        $.ajax({
            url: 'getAPIData',
            method: 'post',
            type: 'json',
            ...
            success: function (data) {
                var result = JSON.parse(data);
                if (result.status=="200") {
                    taxOption = '<select id="taxType_list" class="form-control state_edit" required>';
                    taxOption += '<option value="" selected="selected">--Select Tax Type--</option>';
                ...
                $('.taxType_setup').html(taxOption);
            }
        });
    }

    function getDepositEdit(code_id){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (result.status=="200") {
                    ...
                    $('#taxDepositType').val(detail.taxDepositCode);
                    $('#taxType_list').val(detail.taxTypeCode).trigger('change');
                } else {
                    flash('warning', result.message);
                }
        ...
        function stateView() {
            ...
            var taxDepositType = ($('#taxDepositType').val() == '' ? '-' : $('#taxDepositType').val());
            var taxType = ($('#taxType_list option:selected').text() == '' ? '-' : $('#taxType_list option:selected').text());
            ...
            $('#name_view').text(name);
            $('#dscp_view').text(dscp);
            $('#taxDepositType_view').text(taxDepositType);
            $('#taxType_setup_view').text(taxType);
            $('#save_screen').hide();
            $('.help-block').hide();
            $('#done').hide();
            $('#next_user').hide();
